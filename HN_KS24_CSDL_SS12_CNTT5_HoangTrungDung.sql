...
as select StudentID, FullName , DeptID from Student;

SELECT * FROM studentdb.view_studentbasic;

#Câu 2: Tạo Regular Index cho cột FullName của bảng Student.
create index idx_fullName_student on Student(FullName);
...
begin
select s.*, DeptName from Student s
join Department d
...
as
select d.DeptName, count(s.StudentID) highest
from Student s
join Department d on s.DeptID = d.DeptID
group by s.DeptID;

SELECT * FROM studentdb.view_studentcountbydept;

-- Từ View trên, viết truy vấn hiển thị khoa có nhiều sinh viên nhất.
select max(v.highest) from studentdb.view_studentcountbydept v;
